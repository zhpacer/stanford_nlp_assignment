...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embeding_tmp = tf.nn.embedding_lookup(self.pretrained_embeddings,self.input_placeholder)
        embeddings = tf.reshape(embeding_tmp,[-1,self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def train_on_batch(self, sess, inputs_batch, labels_batch):
        ...
        #print sess.run(self.add_prediction_op(),feed_dict=feed)
    ...
    def __init__(self, config, pretrained_embeddings):
        ...
        self.W = tf.Variable(xavier_initializer([self.config.n_features*self.config.embed_size, self.config.hidden_size]))
        self.b1 = tf.Variable(xavier_initializer([self.config.hidden_size,]))
        self.U = tf.Variable(xavier_initializer([self.config.hidden_size, self.config.n_classes]))
        self.b2 = tf.Variable(xavier_initializer([self.config.n_classes]))        
        self.build()
