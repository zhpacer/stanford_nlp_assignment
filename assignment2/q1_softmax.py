...
def softmax(x):
    ...
    ### END YOUR CODE
    [row,col] = np.shape(x)
    sum = tf.reduce_sum(tf.exp(x),1,keep_dims=True)
    out = tf.exp(x)
    #for i in xrange(col):
        #sum = tf.expand_dims(sum,0)
    out = tf.exp(x)/sum
...
def cross_entropy_loss(y, yhat):
    ...
    out = -tf.reduce_sum(y*tf.log(tf.to_float(yhat)))
    return out
...
def test_cross_entropy_loss_basic():
    ...
    s = tf.Session()
    print s.run(tf.to_float(tf.constant([10,20], dtype=tf.int32)))
    ...
    with tf.Session() as sess:
        test1 = sess.run(test1)
        #mul = -(tf.to_float(y)*tf.log(tf.to_float(yhat)))
        print sess.run(tf.to_float(tf.constant(y, dtype=tf.int32)))
    expected = -3 * np.log(.5)
